...
interface Simulation {
  ...
  tasa_confirmacion_manual: number | null;
...
function SimuladorContent() {
  ...
  const [costoEnvioCod, setCostoEnvioCod] = useState(7.4);
  const [cpaPromedio, setCpaPromedio] = useState(5.0);
  const [tasaEntrega, setTasaEntrega] = useState(0.75); // 75%
  const [costoRechazo, setCostoRechazo] = useState(14.0);
  const [costoFulfillment, setCostoFulfillment] = useState(0.0);
  const [tasaConfirmacion, setTasaConfirmacion] = useState(1.0); // 100%
  // Budget Optimizer
  const [budget, setBudget] = useState(100.0);
  const [strategy, setStrategy] = useState<
    "roi" | "diversified" | "volume" | "portfolio"
  >("roi");
  const [numProducts, setNumProducts] = useState(5);
  const [minBudgetMode, setMinBudgetMode] = useState<
    "none" | "cpa_1.5" | "cpa_2.0" | "custom"
  >("cpa_1.5");
  const [minBudgetCustomVal, setMinBudgetCustomVal] = useState(15.0);
  const [excessDistribution, setExcessDistribution] = useState<
    "best_roi" | "proportional"
  >("best_roi");
  ...
  const [rowScaleSettings, setRowScaleSettings] = useState<
    Record<string, { mode: "pedidos" | "presupuesto"; value: number }>
  >({});
  ...
  const [sortBy, setSortBy] = useState<
    "nombre" | "expectedProfit" | "roi" | "dailyProfit"
  >("nombre");
  const [sortOrder, setSortOrder] = useState<"asc" | "desc">("asc");

  function handleSort(
    field: "nombre" | "expectedProfit" | "roi" | "dailyProfit",
  ) {
    if (sortBy === field) {
  ...
  async function fetchSimulations() {
    ...
    try {
      ...
      if (foundStore && foundStore.costo_rechazo !== undefined) {
        ...
        setCostoRechazo(isNaN(cost) ? 14.0 : cost);
      }
    } catch (e) {
      console.error(e);
    } finally {
  ...
  function handleSelectChange(id: string) {
    ...
    if (!id) {
      ...
      setUnidades(4);
      setCostoEnvioCod(7.4);
      setCpaPromedio(5.0);
      setTasaEntrega(0.75);
      setCostoRechazo(14.0);
      setCostoFulfillment(0.0);
      setTasaConfirmacion(1.0);
      setIsAutoTasa(false);
      return;
    }
    ...
    if (sim) {
      ...
      setCostoFulfillment(Number(sim.costo_fulfillment_proveedor) || 0.0);
      setTasaConfirmacion(
        sim.tasa_confirmacion_manual !== null
          ? Number(sim.tasa_confirmacion_manual)
          : 1.0,
      );
    ...
    try {
      const res = await fetch("/api/simulaciones", {
        ...
        body: JSON.stringify({
          ...
          tasa_confirmacion_manual: tasaConfirmacion,
  ...
  async function handleDeleteFromTable(id: string) {
    ...
    try {
      const res = await fetch(
        `/api/simulaciones?id=${id}&store_id=${storeId}`,
        {
          method: "DELETE",
        },
      );
  ...
  function calculateSimStats(s: Simulation) {
    ...
    const costoTotal =
      Number(s.costo_unitario) * Number(s.unidades_por_venta) +
      (Number(s.costo_fulfillment_proveedor) || 0);
    ...
    const tasa =
      s.tasa_entrega_manual !== null ? Number(s.tasa_entrega_manual) : 0.75;
    const confRate =
    ...
    const lossNoConfirm = cpa;

    const pctDelivered = confRate * tasa;
    const pctRejected = confRate * (1 - tasa);
    const pctNoConfirm = 1 - confRate;
    ...
    const breakevenCpa =
      confRate * (tasa * (precio - costoTotal - envio) - (1 - tasa) * rechazo);

    const roas = cpa > 0 ? (pctDelivered * precio) / cpa : 0;
    const roiAds = cpa > 0 ? (expectedProfit / cpa) * 100 : 0;
  ...
  const stats = useMemo(() => {
    ...
    const profitDelivered =
      precioVenta - costoProductoTotal - costoEnvioCod - cpaPromedio;
    ...
    const breakevenCpa =
      tasaConfirmacion *
      (tasaEntrega * (precioVenta - costoProductoTotal - costoEnvioCod) -
        (1 - tasaEntrega) * costoRechazo);

    // Breakeven delivery rate
    const margenEntregado = precioVenta - costoProductoTotal - costoEnvioCod;
    ...
    // Formula strings for breakdown display
    const profitDeliveredFormula = `Precio (${precioVenta.toFixed(2)}€) − Coste Total (${costoProductoTotal.toFixed(2)}€) − Envío (${costoEnvioCod.toFixed(2)}€) − CPA (${cpaPromedio.toFixed(2)}€) = ${profitDelivered.toFixed(2)}€`;
    const lossRejectedFormula = `Coste Rechazo (${costoRechazo.toFixed(2)}€) + CPA (${cpaPromedio.toFixed(2)}€) = ${lossRejected.toFixed(2)}€`;
    const lossNoConfirmFormula = `CPA (${cpaPromedio.toFixed(2)}€) = ${lossNoConfirm.toFixed(2)}€`;

    const contribDel = pctDelivered * profitDelivered;
    const contribRej = pctRejected * lossRejected;
    const contribNoc = pctNoConfirm * lossNoConfirm;

    const expectedProfitFormulaHTML =
      `✓ ${Math.round(pctDelivered * 100)}% × ${profitDelivered.toFixed(2)}€ → +${contribDel.toFixed(2)}€\n` +
      `✗ ${Math.round(pctRejected * 100)}% × (−${lossRejected.toFixed(2)}€) → −${contribRej.toFixed(2)}€\n` +
      `📱 ${Math.round(pctNoConfirm * 100)}% × (−${lossNoConfirm.toFixed(2)}€) → −${contribNoc.toFixed(2)}€\n` +
      `Total = ${expectedProfit > 0 ? "+" : ""}${expectedProfit.toFixed(2)}€/pedido`;

    const roasFormulaHTML = `${Math.round(pctDelivered * 100)}% × ${precioVenta.toFixed(2)}€ / ${cpaPromedio.toFixed(2)}€ = ${roas.toFixed(1)}x`;
    const roiFormulaHTML = `${expectedProfit > 0 ? "+" : ""}${expectedProfit.toFixed(2)}€ / ${cpaPromedio.toFixed(2)}€ = ${roiAds > 0 ? "+" : ""}${roiAds.toFixed(0)}%`;

    let marginStatus: "profitable" | "tight" | "loss" = "loss";
    if (expectedProfit > 1.5) {
      marginStatus = "profitable";
    } else if (expectedProfit >= 0.0) {
      marginStatus = "tight";
    }

    return {
      costoProductoTotal: Math.round(costoProductoTotal * 100) / 100,
      profitDelivered: Math.round(profitDelivered * 100) / 100,
      lossRejected: Math.round(lossRejected * 100) / 100,
      lossNoConfirm: Math.round(lossNoConfirm * 100) / 100,
      expectedProfit: Math.round(expectedProfit * 100) / 100,
      breakevenCpa: Math.round(breakevenCpa * 100) / 100,
      breakevenEntrega,
      roas: Math.round(roas * 100) / 100,
      roiAds: Math.round(roiAds * 100) / 100,
      cpaCushion: Math.round(cpaCushion * 100) / 100,
      pctDelivered,
      pctRejected,
      pctNoConfirm,
      profitDeliveredFormula,
      lossRejectedFormula,
      lossNoConfirmFormula,
      expectedProfitFormulaHTML,
      roasFormulaHTML,
      roiFormulaHTML,
      marginStatus,
    };
  ...
  const sortedSimulations = useMemo(() => {
    const computed = simulations.map((s) => {
      ...
      const fulfillmentCost = isSelected
        ? costoFulfillment
        : Number(s.costo_fulfillment_proveedor) || 0;
      ...
      const breakevenCpa =
        confRate *
        (tasa * (precio - costoTotal - envCOD) - (1 - tasa) * rechazoCost);

      const roas = cpaVal > 0 ? (pctDelivered * precio) / cpaVal : 0;
      const adsRoi = cpaVal > 0 ? (expectedProfit / cpaVal) * 100 : 0;
      ...
      const confFormatted = isSelected
        ? `${Math.round(confRate * 100)}%`
        : s.tasa_confirmacion_manual !== null
          ? `${Math.round(confRate * 100)}%`
          : "100%";
  ...
  const optimizedAllocation = useMemo(() => {
    ...
    if (strategy === "roi") {
      ...
      if (excess > 0 && supportedCandidates.length > 0) {
        if (excessDistribution === "best_roi") {
          ...
          const totalRoiOfSupported = supportedCandidates.reduce(
            (sum, item) => sum + item.adsRoi,
            0,
          );
          supportedCandidates.forEach((item) => {
            const weight =
              totalRoiOfSupported > 0 ? item.adsRoi / totalRoiOfSupported : 0;
    ...
    const totalDailyProfit = allocations.reduce(
      (sum, item) => sum + item.profitProjected,
      0,
    );
    const totalOrders = allocations.reduce(
      (sum, item) => sum + item.ordersProjected,
      0,
    );
    const overallRoi =
      totalSpent > 0 ? (totalDailyProfit / totalSpent) * 100 : 0;
  ...
  const totals = useMemo(() => {
    ...
    const overallRoi =
      totalAdsSpend > 0 ? (totalDailyProfit / totalAdsSpend) * 100 : 0;
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Analizador avanzado de rentabilidad y unit economics por envío
            </p>
          </div>
        </div>
      </div>
            ...
            <div className="bg-card border rounded-xl p-5 shadow-sm relative overflow-hidden transition-all duration-300 mb-6">
              <div className="flex items-center justify-between border-b pb-3.5 mb-5">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-bold text-xs text-card-foreground">
                    {selectedId
                      ? `⚙️ Editar Parámetros de: ${nombre}`
                      : "➕ Nueva Simulación de Oferta"}
                  </h3>
                </div>
                <button
                  type="button"
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          value={costoEnvioCod}
                          onChange={(e) =>
                            setCostoEnvioCod(Number(e.target.value))
                          }
                          className="w-full text-sm border rounded px-3 py-1.5 bg-background focus:ring-1 focus:ring-primary focus:outline-none"
                        />
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          value={cpaPromedio}
                          onChange={(e) =>
                            setCpaPromedio(Number(e.target.value))
                          }
                          className="w-full text-sm border rounded px-3 py-1.5 bg-background focus:ring-1 focus:ring-primary focus:outline-none"
                        />
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          value={costoRechazo}
                          onChange={(e) =>
                            setCostoRechazo(Number(e.target.value))
                          }
                          className="w-full text-sm border rounded px-3 py-1.5 bg-background focus:ring-1 focus:ring-primary focus:outline-none"
                        />
                      </div>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-bold text-card-foreground">
                            Tasas de Confirmación y Entrega
                          </span>
                        </div>
                        ...
                        <div className="text-xs text-muted-foreground space-y-1.5">
                          {autoTasaLoading ? (
                            <div className="flex items-center gap-2">
                              ...
                              <span>
                                Analizando pedidos en base de datos...
                              </span>
                            </div>
                            ...
                            <div className="flex flex-col gap-1 bg-primary/5 border border-primary/10 rounded-lg p-2.5">
                              <span className="font-semibold text-primary">
                                ✓ ¡Tasa cargada automáticamente!
                              </span>
                              <span>
                                Se encontraron{" "}
                                <strong>{realStats.total_pedidos}</strong>{" "}
                                pedidos con el nombre <i>"{nombre}"</i>.
                              </span>
                              <span>
                                Tasa de entrega calculada:{" "}
                                <strong>
                            ...
                            <div className="flex items-center gap-1.5 text-amber-600 bg-amber-50 border border-amber-100 rounded-lg p-2.5">
                              ...
                              <span>
                                No se encontraron pedidos con el término{" "}
                                <i>"{nombre}"</i> para calcular la tasa.
                                Introdúcela a mano desactivando el check.
                              </span>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="space-y-3">
                          ...
                          <div>
                            <div className="flex items-center justify-between text-xs font-semibold text-muted-foreground mb-1">
                              <span>📱 Tasa de Confirmación (WhatsApp)</span>
                          ...
                          <div>
                            ...
                            <input
                              ...
                              value={tasaEntrega}
                              onChange={(e) =>
                                setTasaEntrega(Number(e.target.value))
                              }
                              className="w-full h-1.5 bg-secondary rounded-lg appearance-none cursor-pointer accent-primary focus:outline-none"
                            />
                          </div>
                ...
                <div className="lg:col-span-5 flex flex-col gap-6">
                  {/* Main Verdict Card */}
                  <div
                    className={`border rounded-xl p-6 shadow-sm relative overflow-hidden transition-all duration-300 bg-gradient-to-br ${
                  ...
                  >
                    ...
                    <div className="mb-5">
                      ...
                      >
                        ...
                        >
                          {precioVenta > 0
                            ? (
                                (stats.expectedProfit / precioVenta) *
                                100
                              ).toFixed(1)
                            : "0.0"}
                          %
                        </span>
                        <span className="text-[0.65rem] text-muted-foreground font-medium ml-0.5">
                          sobre ventas
                        </span>
                      </div>
                    </div>
                    ...
                    >
                      ...
                      {stats.marginStatus === "tight" &&
                        "🟡 MARGEN MUY AJUSTADO"}
                      {stats.marginStatus === "loss" &&
                        "🔴 PRODUCTO A PÉRDIDAS"}
                    </div>

                    {/* Detailed Breakdown Box (3-scenario) */}
                    <div className="mt-5 bg-muted/20 border rounded-xl p-4 space-y-3">
                      {/* Delivered scenario */}
                      <div>
                        <div className="flex justify-between items-center text-xs font-semibold">
                          <span>
                            ...
                            <span className="font-bold text-emerald-600">
                              {Math.round(stats.pctDelivered * 100)}%
                            </span>
                            )
                          </span>
                          <span className="text-emerald-600 font-bold">
                            +{stats.profitDelivered.toFixed(2)}€
                          </span>
                        </div>
                        <div className="text-[0.625rem] text-muted-foreground/70 text-right mt-0.5">
                          {stats.profitDeliveredFormula}
                        </div>
                      </div>
                      {/* Rejected scenario */}
                        ...
                        <div className="flex justify-between items-center text-xs font-semibold">
                          <span>
                            ...
                            <span className="font-bold text-destructive">
                              {Math.round(stats.pctRejected * 100)}%
                            </span>
                            )
                          </span>
                          <span className="text-destructive font-bold">
                            -{stats.lossRejected.toFixed(2)}€
                          </span>
                        </div>
                        <div className="text-[0.625rem] text-muted-foreground/70 text-right mt-0.5">
                          {stats.lossRejectedFormula}
                        </div>
                      </div>
                      {/* No confirm scenario */}
                      <div className="border-t border-dashed pt-3">
                        <div className="flex justify-between items-center text-xs font-semibold">
                          <span>
                            ...
                            <span className="font-bold text-amber-600">
                              {Math.round(stats.pctNoConfirm * 100)}%
                            </span>
                            )
                          </span>
                          <span className="text-destructive font-bold">
                            -{stats.lossNoConfirm.toFixed(2)}€
                          </span>
                        </div>
                        <div className="text-[0.625rem] text-muted-foreground/70 text-right mt-0.5">
                          {stats.lossNoConfirmFormula}
                        </div>
                      </div>
                      {/* Expected profit formula */}
                      <div className="border-t border-dashed pt-3">
                        <div className="text-[0.625rem] text-muted-foreground leading-relaxed whitespace-pre-line font-mono">
                          {stats.expectedProfitFormulaHTML}
                        </div>
                      </div>
                    </div>
                  </div>

                  {/* CPA, ROAS, Breakeven Delivery Rate */}
                  <div className="bg-card border rounded-xl p-5 space-y-4 shadow-sm">
                    <div className="flex items-center gap-1.5">
                      ...
                      <h4 className="text-xs font-bold text-card-foreground">
                        Análisis de Publicidad (Meta Ads)
                      </h4>
                    </div>

                    <div className="space-y-4">
                      {/* CPA Actual + CPA Equilibrio */}
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="bg-muted/30 border rounded-lg p-2.5 space-y-0.5">
                          ...
                          <span className="text-base font-extrabold text-primary">
                            {stats.breakevenCpa.toFixed(2)}€
                          </span>
                          <div className="text-[0.625rem] text-muted-foreground/70 mt-0.5">
                            CPA máximo que puedes pagar sin perder dinero. Si tu
                            CPA real está por debajo, tienes colchón.
                          </div>
                        </div>
                      </div>

                      {/* Tasa de Entrega Actual + Equilibrio */}
                      <div className="grid grid-cols-2 gap-4">
                        <div className="bg-muted/30 border rounded-lg p-2.5 space-y-0.5">
                          <span className="block text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">
                            Tasa de Entrega
                        ...
                        <div className="bg-muted/30 border rounded-lg p-2.5 space-y-0.5">
                          <span className="block text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">
                            Entrega de Equilibrio (mínima)
                          </span>
                          <span
                            className={`text-base font-extrabold ${
                              stats.breakevenEntrega > 1
                                ? "text-destructive"
                                : tasaEntrega >= stats.breakevenEntrega
                                  ? "text-emerald-600"
                                  : "text-destructive"
                            }`}
                          >
                            ...
                            precioVenta -
                              ...
                              : stats.breakevenEntrega > 1
                                ? ">100%"
                                : (stats.breakevenEntrega * 100).toFixed(0) +
                                  "%"}
                          </span>
                          <div className="text-[0.625rem] text-muted-foreground/70 mt-0.5">
                            {tasaConfirmacion <= 0 ||
                            precioVenta -
                              ...
                              costoRechazo <=
                              0
                              ? "Con esta confirmación o margen no se puede calcular un punto de equilibrio."
                              : stats.breakevenEntrega > 1
                                ? "Ni entregando el 100% es rentable. El problema es el producto (precio, coste o CPA), no la entrega."
                                : tasaEntrega >= stats.breakevenEntrega
                                  ? `Tu entrega (${(tasaEntrega * 100).toFixed(0)}%) está ${((tasaEntrega - stats.breakevenEntrega) * 100).toFixed(0)} puntos por encima del mínimo. Tienes colchón.`
                                  : `Tu entrega (${(tasaEntrega * 100).toFixed(0)}%) está ${((stats.breakevenEntrega - tasaEntrega) * 100).toFixed(0)} puntos por debajo del mínimo. Estás perdiendo.`}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="bg-muted/30 border rounded-lg p-2.5 space-y-0.5">
                          <span className="block text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">
                            ROAS (Retorno en Anuncios)
                          </span>
                          <span className="text-base font-extrabold text-primary">
                            {stats.roas.toFixed(1)}x
                          </span>
                          <div className="text-[0.625rem] text-muted-foreground/70 mt-0.5">
                            {stats.roasFormulaHTML}
                          </div>
                        </div>
                        <div className="bg-muted/30 border rounded-lg p-2.5 space-y-0.5">
                          <span className="block text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">
                            ROI Ads (Retorno Total)
                          </span>
                          <span
                            className={`text-base font-extrabold ${stats.roiAds > 0 ? "text-emerald-600" : "text-destructive"}`}
                          >
                            {stats.roiAds > 0 ? "+" : ""}
                            {stats.roiAds.toFixed(0)}%
                          </span>
                          <div className="text-[0.625rem] text-muted-foreground/70 mt-0.5">
                            {stats.roiFormulaHTML}
                          </div>
                        </div>
                      </div>
                      ...
                      {/* CPA Cushion Alert */}
                      <div
                        className={`rounded-xl p-3 border text-xs flex gap-2.5 items-start ${
                        ...
                        <div>
                          {stats.breakevenCpa > cpaPromedio ? (
                            <div>
                              ...
                              <span>
                                Tienes{" "}
                                <strong>+{stats.cpaCushion.toFixed(2)}€</strong>{" "}
                            ...
                            <div>
                              ...
                              <span>
                                Estás pagando{" "}
                                <strong>{cpaPromedio.toFixed(2)}€</strong> de
                                CPA, pero tu límite es{" "}
                                <strong>
                                  {stats.breakevenCpa.toFixed(2)}€
                                </strong>
                                . ¡Necesitas bajar el CPA o subir la tasa de
                                confirmación o entrega!
                              </span>
                            </div>
                          )}
                        </div>
                      ...
                      <div className="text-right">
                        <button
                          ...
                          className="text-[0.7rem] text-muted-foreground underline hover:text-card-foreground transition-colors"
                        >
                          🤔 Pedido en duda
                        </button>
                      </div>
          ...
          {simulations.length > 0 && (
            <div className="bg-card border rounded-xl p-5 shadow-sm mt-6 space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b pb-3.5">
                <div>
                  <h3 className="font-bold text-xs text-card-foreground">
                    📋 Comparativa de Productos Simulados
                  </h3>
                  <p className="text-[10px] text-muted-foreground mt-0.5">
                    Haz clic en una fila para editar sus variables base,
                    marca/desmarca el checkbox del nombre para incluir/excluir
                    en la optimización automática de anuncios, o **escribe
                    directamente** en las columnas "Simulado" de cada fila para
                    proyectar diferentes volúmenes independientes.
                  </p>
                </div>
                {!isFormOpen && (
                  <button
                  ...
                  >
                    <Sparkles className="size-3.5 animate-pulse" />➕ Nueva
                    Oferta / Simulación
                  </button>
                )}
              </div>
                ...
                <table className="w-full text-xs text-left border-collapse">
                  <thead>
                    <tr className="border-b bg-muted/40 font-semibold text-muted-foreground select-none">
                      ...
                      <th className="py-2.5 px-3 text-center">Tasa Confirma</th>
                      <th className="py-2.5 px-3 text-right">CPA Límite</th>
                      <th className="py-2.5 px-3 text-right text-emerald-600 font-semibold bg-emerald-50/5">
                        Si se Entrega
                      </th>
                      <th className="py-2.5 px-3 text-right text-destructive font-semibold bg-destructive/5">
                        Si se Rechaza
                      </th>
                      <th
                      ...
                      {/* Projection Headers */}
                      <th className="py-2.5 px-3 text-center bg-muted/20 text-primary font-bold">
                        Simulado: Pedidos
                      </th>
                      <th className="py-2.5 px-3 text-right bg-muted/20 text-primary font-bold">
                        Simulado: Gasto Ads
                      </th>
                      <th
                  ...
                  <tbody className="divide-y">
                    {sortedSimulations.map((item) => {
                      ...
                      return (
                        ...
                        >
                          <td
                            className="py-3 px-3 text-card-foreground font-medium truncate max-w-[150px]"
                          ...
                          >
                            <div className="flex items-center gap-2">
                              <input
                                ...
                                title={
                                  excludedProductIds[s.id]
                                    ? "Incluir en optimizador"
                                    : "Excluir de optimizador"
                                }
                              />
                              <span
                                onClick={() => handleSelectChange(s.id)}
                                className={`cursor-pointer hover:underline ${
                                  excludedProductIds[s.id]
                                    ? "line-through text-muted-foreground/50 font-normal"
                                    : ""
                                }`}
                              >
                                {item.nombre}
                          ...
                          <td className="py-3 px-3 text-right text-muted-foreground">
                            {item.envCOD.toFixed(2)}€
                          </td>
                          <td className="py-3 px-3 text-right text-muted-foreground">
                            {item.cpaVal.toFixed(2)}€
                          </td>
                          ...
                          <td className="py-3 px-3 text-right font-bold text-card-foreground">
                            {item.breakevenCpa.toFixed(2)}€
                          </td>
                          <td className="py-3 px-3 text-right text-emerald-600 font-semibold bg-emerald-50/5">
                            +{item.profitDelivered.toFixed(2)}€
                          </td>
                          <td className="py-3 px-3 text-right text-destructive font-semibold bg-destructive/5">
                            -{item.lossRejected.toFixed(2)}€
                          </td>
                          <td
                            className={`py-3 px-3 text-right font-black border-r ${
                              item.expectedProfit > 0
                                ? "text-emerald-600 bg-emerald-50/5"
                          ...
                          <td
                            className="py-2.5 px-3 bg-muted/5 text-center"
                            onClick={(e) => e.stopPropagation()}
                          >
                            <div className="flex items-center justify-center gap-1">
                              <input
                                ...
                                step="1"
                                value={
                                  item.projectedOrders % 1 === 0
                                    ? item.projectedOrders
                                    : Number(item.projectedOrders.toFixed(1))
                                }
                                onChange={(e) => {
                                  const val = Math.max(
                                    0,
                          ...
                          >
                            <div className="flex items-center justify-center gap-1">
                              <input
                                ...
                                step="1"
                                value={
                                  item.projectedAdsSpend % 1 === 0
                                    ? item.projectedAdsSpend
                                    : Number(item.projectedAdsSpend.toFixed(2))
                                }
                                onChange={(e) => {
                                  const val = Math.max(
                                    0,
                          ...
                          <td
                            className={`py-3 px-3 text-right font-bold bg-muted/5 ${
                              item.projectedDailyProfit > 0
                                ? "text-emerald-600 bg-emerald-50/5"
                                : "text-destructive bg-destructive/5"
                            }`}
                          >
                            {item.projectedDailyProfit > 0 ? "+" : ""}
                            {item.projectedDailyProfit.toFixed(2)}€
                          </td>
                          <td
                            className={`py-3 px-3 text-right font-black bg-muted/5 ${
                              item.projectedDailyProfit > 0
                                ? "text-emerald-600 bg-emerald-50/5"
                                : "text-destructive bg-destructive/5"
                            }`}
                          >
                            {item.projectedDailyProfit * 30 > 0 ? "+" : ""}
                            {(item.projectedDailyProfit * 30).toFixed(2)}€
                          </td>
                          <td className="py-3 px-3 text-center bg-muted/5">
                            ...
                            >
                              {item.adsRoi > 0 ? "+" : ""}
                              {item.adsRoi.toFixed(1)}% ROI
                            </span>
                          </td>

                          <td
                            className="py-3 px-3 text-center flex items-center justify-center gap-1"
                            onClick={(e) => e.stopPropagation()}
                          >
                            <button
                              onClick={() => {
                    ...
                    <tr className="bg-primary/5 font-extrabold border-t-2 border-double border-primary/20">
                      <td className="py-3 px-3 text-primary font-bold text-xs">
                        TOTAL SIMULADO
                      </td>
                      <td className="py-3 px-3 text-right text-muted-foreground">
                        -
                      ...
                      <td className="py-3 px-3 text-right text-muted-foreground border-r font-medium">
                        -
                      </td>

                      {/* Projected scaling totals */}
                      <td className="py-3 px-3 text-center text-primary bg-primary/10 font-bold">
                        {totals.totalOrders % 1 === 0
                          ? totals.totalOrders
                          : totals.totalOrders.toFixed(1)}{" "}
                        /día
                      </td>
                      ...
                      <td
                        className={`py-3 px-3 text-right font-black bg-primary/10 ${
                      ...
                      >
                        {totals.totalDailyProfit > 0 ? "+" : ""}
                        {totals.totalDailyProfit.toFixed(2)}€
                      </td>
                      <td
                        className={`py-3 px-3 text-right font-black bg-primary/10 ${
                          totals.totalMonthlyProfit > 0
                            ? "text-emerald-600"
                            : "text-destructive"
                        }`}
                      >
                        {totals.totalMonthlyProfit > 0 ? "+" : ""}
                        {totals.totalMonthlyProfit.toFixed(2)}€
                      </td>
                      <td className="py-3 px-3 text-center bg-primary/10">
                        ...
                        >
                          {totals.overallRoi > 0 ? "+" : ""}
                          {totals.overallRoi.toFixed(1)}% ROI
                        </span>
                      </td>
                      <td className="py-3 px-3 text-center text-muted-foreground">
                        -
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b pb-4">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-muted-foreground mt-0.5">
                      Distribuye tu presupuesto diario de publicidad de manera
                      óptima entre tus ofertas ganadoras.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <div className="md:col-span-4 space-y-4">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      />
                      <span className="absolute left-3 top-2 text-muted-foreground text-sm font-semibold">
                        €
                      </span>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    >
                      <option value="roi">
                        📈 Rentabilidad Máxima (Best ROI)
                      </option>
                      <option value="portfolio">
                        💼 Cartera Optimizada (Top N)
                      </option>
                    ...
                    <div className="space-y-3 pt-3 border-t border-dashed">
                      <div>
                        ...
                        <input
                          ...
                          value={numProducts}
                          onChange={(e) =>
                            setNumProducts(Math.max(1, Number(e.target.value)))
                          }
                      ...
                      <div>
                        ...
                        <select
                          value={minBudgetMode}
                          onChange={(e) =>
                            setMinBudgetMode(e.target.value as any)
                          }
                        ...
                        >
                          <option value="cpa_1.5">
                            ⚡ Automático (1.5x CPA)
                          </option>
                          <option value="cpa_2.0">
                            🔥 Automático (2.0x CPA)
                          </option>
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setExcessDistribution(e.target.value as any)
                          }
                          className="w-full text-sm border rounded px-3 py-1.5 bg-background focus:ring-1 focus:ring-primary focus:outline-none font-medium cursor-pointer"
                        >
                          <option value="best_roi">
                            ⭐ Concentrar en Mejor ROI
                          </option>
                          <option value="proportional">
                            ⚖️ Proporcional al ROI
                          </option>
                        </select>
                      </div>
                    </div>
                  ...
                  <div className="pt-2 border-t">
                    <div className="text-[10px] text-muted-foreground leading-relaxed">
                      * El optimizador matemático **excluye** automáticamente
                      los productos a pérdidas o con ROI negativo para proteger
                      tu capital de anuncios.
                    </div>
                  </div>
                </div>
                ...
                <div className="md:col-span-8 space-y-6">
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    <div className="bg-muted/30 border rounded-xl p-3.5 space-y-1">
                      ...
                      <span className="text-lg font-extrabold text-card-foreground">
                        {optimizedAllocation.totalSpent.toFixed(2)}€
                      </span>
                    </div>
                    <div className="bg-muted/30 border rounded-xl p-3.5 space-y-1">
                      <span className="block text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">
                      ...
                      <span
                        className={`text-lg font-black ${
                          optimizedAllocation.totalDailyProfit > 0
                            ? "text-emerald-600"
                  ...
                  <div className="space-y-3">
                      ...
                      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-xs text-amber-800 flex gap-2.5 items-start mb-4">
                        ...
                        <div>
                          <span className="font-bold block text-amber-900 mb-0.5">
                            ⚠️ Presupuesto Ajustado / Insuficiente
                          </span>
                          <span>
                            Tu presupuesto diario de{" "}
                            <strong>{budget.toFixed(2)}€</strong> no es
                            suficiente para cubrir el presupuesto mínimo viable
                            de los{" "}
                            <strong>{optimizedAllocation.targetCount}</strong>{" "}
                            productos solicitados. Hemos priorizado y activado
                            únicamente los{" "}
                            <strong>{optimizedAllocation.activeCount}</strong>{" "}
                            productos de mayor ROI. Sube el presupuesto diario o
                            reduce el mínimo por campaña para activar más
                            productos.
                          </span>
                        </div>
                      </div>
                    ...
                    <h4 className="text-xs font-bold text-card-foreground uppercase tracking-wider">
                      Distribución Recomendada de Ads
                    </h4>
                    {optimizedAllocation.allocations.length === 0 ? (
                      <div className="border border-dashed rounded-xl p-6 text-center text-muted-foreground">
                        ...
                        <span className="text-xs">
                          No tienes productos con rentabilidad esperada positiva
                          en tu catálogo. ¡Ajusta tus precios, cpa o tasas de
                          entrega!
                        </span>
                      </div>
                    ) : (
                        ...
                        <table className="w-full text-xs text-left border-collapse">
                          <thead>
                            <tr className="border-b bg-muted/40 font-semibold text-muted-foreground select-none">
                              ...
                              <th className="py-2.5 px-3.5 text-right text-emerald-600 font-bold">
                                Ganancia Diaria Net
                              </th>
                            </tr>
                          </thead>
                          <tbody className="divide-y">
                              ...
                              >
                                ...
                                <td className="py-3 px-3 text-right font-semibold text-card-foreground">
                                  <div className="flex flex-col items-end">
                                    <span>
                                      {item.budgetAllocated.toFixed(2)}€
                                ...
                                <td className="py-3 px-3 text-right font-medium text-muted-foreground">
                                  {item.ordersProjected.toFixed(1)}
                                  <span className="text-[10px] text-muted-foreground font-normal">
                                    {" "}
                                    ped/día
                                  </span>
                                ...
                                <td className="py-3 px-3.5 text-right font-black text-emerald-600">
                                  +{item.profitProjected.toFixed(2)}€
                            ...
                            <tr className="bg-primary/5 font-extrabold border-t-2 border-double border-primary/20">
                              ...
                              <td className="py-2.5 px-3 text-right text-primary font-bold">
                                {optimizedAllocation.totalOrders.toFixed(1)}
                                <span className="text-[10px] font-normal">
                                  {" "}
                                  /d
                                </span>
                              ...
                              <td className="py-2.5 px-3.5 text-right text-emerald-600 font-black">
                                +
                                {optimizedAllocation.totalDailyProfit.toFixed(
                                  2,
                                )}
                                €
                                <span className="text-[10px] font-normal">
      ...
      {showDoubtModal && (
        ...
        >
          ...
          >
            ...
            <p className="text-xs text-muted-foreground mb-4">
              El cliente está dudando. Quieres llamarlo y ofrecerle un descuento
              para que acepte el contrareembolso. ¿Cuánto puedes bajar sin
              perder más que si lo rechazara?
            </p>
            ...
            {(() => {
              ...
              return (
                <div className="flex justify-between items-center bg-muted/30 border rounded-lg px-3 py-2 mb-3 text-xs">
                  <span>
                    Precio original: <strong>{precioVenta.toFixed(2)}€</strong>
                  </span>
                  <span>
                    −{" "}
                ...
                <>
                  <div className="grid grid-cols-2 gap-3 mb-3">
                    <div className="bg-muted/30 border rounded-lg p-3">
                      <div className="text-[0.65rem] font-semibold text-muted-foreground mb-1">
                  ...
                  <div className="bg-blue-50 border rounded-lg p-3 text-xs space-y-1">
                    <div>
                      ...
                      <span className="text-[0.7rem] text-muted-foreground">
                        (
                        {precioVenta > 0
                          ? ((breakevenDiscount / precioVenta) * 100).toFixed(0)
                          : 0}
                        %)
                      </span>{" "}
                    ...
                    <div className="border-t border-blue-100 pt-1.5 mt-1.5">
                      {costToDeliver > costoRechazo ? (
                        <>
                          ...
                          <strong>{costoRechazo.toFixed(2)}€</strong>. A partir
                          de <strong>{crossDiscountPct.toFixed(0)}%</strong> de
                        ...
                        <>
                          ...
                          <strong>siempre conviene ofrecer descuento</strong>.
                        </>
                      )}
                    </div>
                  </div>
